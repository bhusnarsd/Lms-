...
const getPresentator = catchAsync(async (req, res) => {
  const singleStudio = await presentatorService.getPresentatorById(
    req.params.presentatorId
  );
  if (!singleStudio) {
...
const updatePresentator = catchAsync(async (req, res) => {
  const updatedStudio = await presentatorService.updatePresentatorById(
    req.params.presentatorId,
    req.body
  );
...
const deletePresentator = catchAsync(async (req, res) => {
  const deletedStudio = await presentatorService.deletePresentatorById(
    req.params.presentatorId
  );
